...
public sealed class MattermostChannel : NotificationChannelBase
{
    ...
    /// <inheritdoc />
    public override string ChannelName => "mattermost";

    /// <summary>
    /// Initialises a new instance of <see cref="MattermostChannel"/>.
    /// </summary>
    ...
    {
        _logger.LogDebug(
            "Mattermost: sending message to webhook");
        ...
        try
        {
            ...
            {
                _logger.LogDebug(
                    "Mattermost: send failed. Status {Status}. Body {Body}",
                    (int)response.StatusCode, raw);

                return Fail(payload.To, $"HTTP {(int)response.StatusCode}: {raw}");
            }

            _logger.LogDebug("Mattermost: message sent successfully");
        ...
        {
            _logger.LogDebug(ex, "Mattermost: exception sending message");
...
internal sealed class MattermostWebhookRequest
{
